...
const SocialIcons: React.FC<SocialIconsProps> = ({ variant = 'default', className = '' }) => {
  const iconSizes = {
    default: 'w-6 h-6',
    large: 'w-8 h-8',
    footer: 'w-6 h-6'
  ...
  const iconColors = {
    default: 'text-gray-700 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400',
    large: 'text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300',
    footer: 'text-slate-400 hover:text-emerald-400 dark:hover:text-emerald-300'
  };

  return (
    <div className={`flex space-x-4 ${className}`}>
      <a 
        href="https://www.linkedin.com/in/ajuevan/" 
        target="_blank" 
        rel="noopener noreferrer"
        className={`transition-colors duration-200 ${iconColors[variant]}`}
        aria-label="LinkedIn"
      >
        <svg className={iconSizes[variant]} fill="currentColor" viewBox="0 0 24 24">
